...
#import "SmsManager.h"
@interface JCForgetPsdViewController ()
...
@implementation JCForgetPsdViewController
...
- (void)loginBtnClick {
    ...
    [service forgetPwdWithTel:phone pass:password code:self.codeTF.text success:^(id  _Nullable object) {
        if ([JCWJsonTool isSuccessResponse:object]) {
            [JCWToastTool showHint:@"密码重置成功"];
...
//    forgetPwdWithTel
//    [service changePwdWithTel:weakSelf.phoneTF.text pass:weakSelf.passwordTF.text code:weakSelf.codeTF.text success:^(id  _Nullable object) {
//    } failure:^(NSError * _Nonnull error) {
//        [JCWToastTool showHint:@"密码重置失败"];
//    }];
...
- (void)codeBtnClick {
    ...
    BOOL isDebug = NO;
    WeakSelf;
    [[SmsManager sharedManager] getCodeWithTel:weakSelf.phoneTF.text type:SMSCodeTypeChange isDebug:isDebug callback:^(BOOL isSucceed, NSString * _Nonnull code) {
        if (isDebug) {
...
- (UIButton *)loginBtn {
    if (!_loginBtn) {
        _loginBtn = [UIButton initWithText:@"提交修改" FontSize:AUTO(16) Weight:2 BackGroundColor:JCBaseColor TextColors:JCWhiteColor];
