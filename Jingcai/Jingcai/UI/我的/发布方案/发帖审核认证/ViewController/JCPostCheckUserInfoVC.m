...
#import "JCWAppTool.h"
@interface JCPostCheckUserInfoVC ()<UITextFieldDelegate>
...
@implementation JCPostCheckUserInfoVC
...
- (void)nextBtnClick {
    NSString *account = [self.account stringByReplacingOccurrencesOfString:@" " withString:@""];
    if (account.length<30) {
        [JCWToastTool showHint:@"您输入的申请理由纯文本内容不足30字，请重新输入！"];
        return;
    }
//    JCPostCheckArticleVC *vc = [JCPostCheckArticleVC new];
    ...
    self.checkVC.account = self.account;
...
- (void)configData {
    ...
    BOOL account_isRight = self.account.length>=30;
    //去除掉空格后,字符大于30
//    NSString *deleteBlank = [self.account stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
//    BOOL blank = deleteBlank.length>=30;

    if (name_isRight&&phone_isRight&&idcard_isRight&&self.headImg&&account_isRight) {
...
- (JCPostCheckArticleVC *)checkVC {
    if (!_checkVC) {
        _checkVC = [JCPostCheckArticleVC new];
    }
    return _checkVC;
}
