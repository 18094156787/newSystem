...
- (void)setTitleModel:(JCActivityGoodsTitleModel *)titleModel {
    ...
    if ([titleModel.grade integerValue]>0&&[self.detailModel.type integerValue]==3&&[self.detailModel.pay_type integerValue]==2) {
        ...
        self.titleLab.text = [NSString stringWithFormat:@"累计积分%@元",NonNil(titleModel.grade)];
        self.statusLab.hidden = [titleModel.is_own integerValue]==1?NO:YES;
    }else{
        self.titleLab.text = @"";
...
- (UILabel *)statusLab {
    if (!_statusLab) {
        _statusLab = [UILabel initWithTitle:@"已领取" andFont:AUTO(14) andWeight:2 andTextColor:UIColorFromRGB(0x278A66) andBackgroundColor:JCClearColor andTextAlignment:0];
