...
@property (nonatomic,strong) JCActivityGuessCheckView *checkView;
...
- (void)initSubViews {
    ...
    self.checkView.JCSureBlock = ^{
        ...
        [weakSelf finalSubmitWithDataArray:weakSelf.checkView.dataSource];
    };
}
...
- (void)showCheckView {
    ...
    self.checkView.dataSource  = array;
    [self.jcWindow addSubview:self.checkView];
}

- (void)finalSubmitWithDataArray:(NSMutableArray *)dataArray {
    NSString *options = @"";
...
- (JCActivityGuessCheckView *)checkView {
    if (!_checkView) {
        _checkView = [JCActivityGuessCheckView new];
