...
- (void)buttonClick:(UIButton *)sender {
    ...
    if (self.matchModel.status_id>8) {
        [JCWToastTool showHint:@"比赛状态异常，无法提交！"];
        return;
    }
    if (self.matchModel.status_id!=1) {
...
//        self.matchModel.local_choise = NO;
    ...
    if (self.matchModel.activity_option_info.count>0) {
...
//    }
    ...
    [self.homeImgView sd_setImageWithURL:[NSURL URLWithString:matchModel.home_team_logo] placeholderImage:JCIMAGE(@"home_placeholder")];
    [self.awayImgView sd_setImageWithURL:[NSURL URLWithString:matchModel.away_team_logo] placeholderImage:JCIMAGE(@"away_placeholder")];
    self.homeLab.text = matchModel.home_team_name;
    self.awayLab.text = matchModel.away_team_name;
    self.statusLab.text = matchModel.status_cn;
    self.scoreLab.text = @"VS";
    
    if (matchModel.status_id>1&&matchModel.status_id<8) {
        self.scoreLab.text = [NSString stringWithFormat:@"%@:%@",matchModel.home_scores,matchModel.away_scores];
        
        if ([matchModel.second_half_time floatValue]>0) {
            //计算时间差
            double currentTime = [[NSDate date] timeIntervalSince1970];
            double distance = currentTime-[matchModel.second_half_time floatValue];
...
//            [NSDate dateWithTimeIntervalSince1970:(NSTimeInterval)]
            ...
            //计算时间差
            double currentTime = [[NSDate date] timeIntervalSince1970];
            double distance = currentTime-[matchModel.first_half_time floatValue];
            if (distance<0) {
                distance = 0;
            }
