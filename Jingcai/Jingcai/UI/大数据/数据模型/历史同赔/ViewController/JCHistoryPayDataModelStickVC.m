//
//  JCHistoryPayDataModelStickVC.m
//  Jingcai
//
...
@interface JCHistoryPayDataModelStickVC ()
...
@property (nonatomic,strong) JCHistoryPayDataModelVC *dataVC;


@end

@implementation JCHistoryPayDataModelStickVC
...
//    NSString *day =  [NSString stringWithFormat:@"%@",self.buyInfoModel.free_day];
    NSString *title = [NSString stringWithFormat:@"是否开通[历史同赔] %@天免费体验",self.buyInfoModel.free_day];
...
//确认购买
- (void)sureBuy {
    if (self.buyInfoModel.big_data_price>0&&[[JCWUserBall currentUser].prize floatValue]<self.buyInfoModel.big_data_price/100) {
...
//    NSLog(@"%.0f--%.0f",scrollView.contentOffset.y,max);
}
...
- (JCHistoryPayDataModelVC *)dataVC {
    if (!_dataVC) {
        _dataVC = [JCHistoryPayDataModelVC new];
        WeakSelf;
        _dataVC.JCOpenBlock = ^{
            [weakSelf payAction];
        };
