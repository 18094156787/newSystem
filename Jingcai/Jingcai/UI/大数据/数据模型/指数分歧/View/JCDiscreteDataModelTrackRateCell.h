//
//  JCDiscreteDataModelTrackRateCell.h
//  Jingcai
//
//  Created by 陈继伟 on 2021/11/11.
//  Copyright © 2021 blockstar. All rights reserved.
//

#import "JCBaseTableViewCell_New.h"
#import "JCDiscreteTrackRateModel.h"
NS_ASSUME_NONNULL_BEGIN

@interface JCDiscreteDataModelTrackRateCell : JCBaseTableViewCell_New

@property (nonatomic, strong) UILabel *winLab;

@property (nonatomic, strong) UILabel *equalLab;

@property (nonatomic, strong) UILabel *loseLab;

@property (nonatomic, strong) UILabel *timeLab;

@property (nonatomic, strong) JCDiscreteTrackRateModel *model;

@end

NS_ASSUME_NONNULL_END
