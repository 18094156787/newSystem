...
- (void)initViews {
    ...
    [topView addSubview:self.away_winLab];
    ...
    [topView addSubview:self.away_equalLab];
    ...
    [topView addSubview:self.away_loseLab];
    ...
    [self.home_rateLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.offset(0);
        make.bottom.equalTo(topView.mas_centerY);
        make.left.equalTo(self.home_zs_LoseLab.mas_right);
        make.right.offset(AUTO(-15));
    }];
    ...
    [topView addSubview:self.away_rateLab];
...
- (void)setModel:(JCKellyDataDetailSampleModel *)model {
    ...
    [model.begin_odds.odds_rate enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx==0) {
    ...
    [model.last_odds.odds_rate enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx==0) {
