...
@interface JCDiscreteDataModelStickVC ()
...
@property (nonatomic,strong) JCDiscreteDataModelVC *dataVC;

@end

@implementation JCDiscreteDataModelStickVC
...
//    NSString *day =  [NSString stringWithFormat:@"%@",self.buyInfoModel.free_day];
    NSString *title = [NSString stringWithFormat:@"是否开通[指数分歧] %@天免费体验",self.buyInfoModel.free_day];
...
//    [self.navigationController.view addSubview:self.topColorView];
    ...
    UILabel *titleLab = [UILabel initWithTitle:@"指数分歧" andFont:17 andWeight:3 andTextColor:COLOR_2F2F2F andBackgroundColor:JCClearColor andTextAlignment:NSTextAlignmentCenter];
...
- (JCDiscreteDataModelVC *)dataVC {
    if (!_dataVC) {
        _dataVC = [JCDiscreteDataModelVC new];
        WeakSelf;
        _dataVC.JCOpenBlock = ^{
            [weakSelf payAction];
        };
