...
//    self.matchNameLab.textColor = model.competition_color.length>0?[UIColor colorWithHexString:NonNil(model.competition_color)]:UIColorFromRGB(0x606062);
    self.matchTimeLab.text = model.get_match_time;
    self.matchStatusLab.text = model.status_cn;
    self.homeTeamLab.text = model.home_team.name_zh;
    self.awayTeamLab.text =  model.away_team.name_zh;

    
    [self.homeTeamImgView sd_setImageWithURL:[NSURL URLWithString:model.home_team.logo] placeholderImage:JCIMAGE(@"home_placeholder")];
    [self.awayTeamImgView sd_setImageWithURL:[NSURL URLWithString:model.away_team.logo] placeholderImage:JCIMAGE(@"away_placeholder")];
