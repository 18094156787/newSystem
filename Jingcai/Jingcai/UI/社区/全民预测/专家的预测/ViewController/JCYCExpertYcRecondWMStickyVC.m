//
//  JCYCExpertYcRecondWMStickyVC.m
//  Jingcai
//
...
@interface JCYCExpertYcRecondWMStickyVC ()
...
@implementation JCYCExpertYcRecondWMStickyVC
...
- (void)getUserInfo {
    ...
    [service getUserZhanjiListWithType:@"2" userID:self.expertID page:1 success:^(id  _Nullable object) {
        ...
        if ([JCWJsonTool isSuccessResponse:object]) {
            ...
            self.title = [NSString stringWithFormat:@"%@的战绩",model.user_info.user_name];
//            JCYCUserInfoModel *model = (JCYCUserInfoModel *)[JCWJsonTool entityWithJson:object[@"data"][@"user"] class:[JCYCUserInfoModel class]];
//            self.topHeadView.model = model;
//            self.title = [NSString stringWithFormat:@"%@的战绩",model.user_name];
        }else{
...
- (UIViewController *)pageController:(WMPageController *)pageController viewControllerAtIndex:(NSInteger)index {
    JCYCExpertYcRecordViewController *vc = [JCYCExpertYcRecordViewController new];
...
- (CGRect)pageController:(WMPageController *)pageController preferredFrameForContentView:(WMScrollView *)contentView {
    ...
    return CGRectMake(0, originY, SCREEN_WIDTH, SCREEN_HEIGHT-kBottomTabSafeAreaHeight-50-kWMMenuViewHeight-kNavigationBarHeight);
}
