...
@interface JCYCTenRedRankVC ()

@end

@implementation JCYCTenRedRankVC
...
- (void)getDataList {
    ...
    [service getJingcaiRankList_TenRedWeekWithpage:self.pageNo success:^(id  _Nullable object) {
        [self endRefresh];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    JCYCLianhongRankCell *cell = [tableView dequeueReusableCellWithIdentifier:@"JCYCLianhongRankCell"];
    if (indexPath.row>2) {
        ...
        NSString *imageName = [NSString stringWithFormat:@"jc_yu_rank%ld",indexPath.row];
        cell.rankImgView.image = JCIMAGE(imageName);
    }
    cell.rankLab.text = [NSString stringWithFormat:@"%02ld",indexPath.row+1];
