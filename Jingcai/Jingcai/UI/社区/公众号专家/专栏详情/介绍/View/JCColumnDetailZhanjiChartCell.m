...
- (void)initViews {
    ...
    [self.bgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(AUTO(105+20));
    ...
    for (int i=0; i<5; i++) {
        ...
        lineView.frame = CGRectMake(AUTO(15), AUTO(21)*(i+1), SCREEN_WIDTH-AUTO(30), 0.8);
...
-(void)addCircle{
    self.dataArray = @[@"0",@"0.2",@"0.5",@"1",@"0.2",@"0.3",@"0",@"0.1",@"0.6",@"0.5",@"0.4",@"0"];//,@"0.1",@"0.6",@"0.6",@"0.6",
    
    float width = SCREEN_WIDTH-AUTO(30);
    float height = AUTO(84);
    CAGradientLayer *layer = [CAGradientLayer layer];
    layer.frame = CGRectMake(0, AUTO(20), width, height);
    //渐变的颜色
    ...
    for (int i=0; i<self.dataArray.count; i++) {
        ...
        float startY = [self.dataArray[i] floatValue]*height;
    ...
    [self.bgView.layer addSublayer:layer];
        ...
        for (int i=0; i<self.dataArray.count; i++) {
            ...
            if (i>=1&&i<=(self.dataArray.count-2)) {
                ...
                view.center = CGPointMake(startX, startY+AUTO(20));
                ...
                label.center = CGPointMake(points.x+AUTO(30), 0);
                label.bottom = AUTO(130);
                label.text = [NSString stringWithFormat:@"%d期",i];
            }
...
- (void)tapClick:(UITapGestureRecognizer *)tap {
    ...
    self.showLab.text = [NSString stringWithFormat:@"%@%%",@"75.58"];
