...
@property (nonatomic,strong) JCMatchFilterVC *bdVC;
...
@implementation JCMatchFilterWMVC
...
- (void)configData {
        if (self.hotVC.dataSource.count>0||self.allVC.dataSource.count>0||self.jcVC.dataSource.count>0||self.bdVC.dataSource.count>0) {
            return;
        }
    [self getDataList];
}
...
- (UIViewController *)pageController:(WMPageController *)pageController viewControllerAtIndex:(NSInteger)index {
    if (index==0) {
        self.hotVC.type =  [self.type integerValue];
        self.hotVC.time = self.time;
        return self.hotVC;
    }
    if (index==1) {
        self.allVC.type = [self.type integerValue];
        self.allVC.time = self.time;
        self.allVC.selectIndex = index;
        return self.allVC;
    }
    if (index==2) {
        self.jcVC.type =  [self.type integerValue];
        self.jcVC.time = self.time;
        return self.jcVC;
    }
    self.bdVC.type =  [self.type integerValue];
    self.bdVC.time = self.time;
    return self.bdVC;
...
- (void)configSelectStatus {
    ...
    if (self.selectIndex==3) {
        self.filterBtn.selected = self.bdVC.isSelectAll;
    }
}
...
- (void)filterBtnClick:(UIButton *)sender {
    ...
    if (self.selectIndex==3) {
        [self.bdVC selectAllWithStatus:sender.selected];
    }
    [self selMatchCountWithIndex:self.selectIndex];
...
//        }
    ...
    if (index==3) {
        self.selMatchCount = 0;
        [self.bdVC.dataArr enumerateObjectsUsingBlock:^(JCMatchFilterModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj.isSelect) {
                self.selMatchCount++;
            }
        }];
        self.bdVC.isSelectAll = self.selMatchCount==self.bdVC.dataArr.count?YES:NO;
        self.filterBtn.selected = self.selMatchCount==self.bdVC.dataArr.count?YES:NO;
        self.selMatchLab.text = [NSString stringWithFormat:@"已选择%ld个联赛",self.selMatchCount];
    }
...
- (JCMatchFilterVC *)jcVC {
    if (!_jcVC) {
        ...
        _jcVC.selectIndex = 2;
        WeakSelf;
        _jcVC.JCSelectBlock = ^(NSInteger type) {
            [weakSelf selMatchCountWithIndex:type];
        };
    }
    return _jcVC;
...
- (JCMatchFilterVC *)bdVC {
    if (!_bdVC) {
        _bdVC = [JCMatchFilterVC new];
        _bdVC.style = 1;
        _bdVC.selectIndex = 3;
        WeakSelf;
        _bdVC.JCSelectBlock = ^(NSInteger index) {
