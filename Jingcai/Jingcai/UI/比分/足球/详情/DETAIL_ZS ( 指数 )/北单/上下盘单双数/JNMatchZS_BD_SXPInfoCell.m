...
#import "JNMatchZS_BD_SXPInfoCell.h"

@implementation JNMatchZS_BD_SXPInfoCell
...
- (void)setModel:(JNMatchSXPBall *)model {
    _model = model;
    self.timeLab.text = model.created_desc;
    self.sdLab.text = model.sx11;
    self.ssLab.text = model.sx10;
    self.xdLab.text = model.sx01;
    self.xsLab.text = model.sx00;
}
...
- (UILabel *)sdLab {
    if (!_sdLab) {
        _sdLab = [UILabel initWithTitle:@"" andFont:AUTO(10) andWeight:1 andTextColor:COLOR_2F2F2F andBackgroundColor:JCClearColor andTextAlignment:NSTextAlignmentCenter];
...
- (UILabel *)ssLab {
    if (!_ssLab) {
        _ssLab = [UILabel initWithTitle:@"" andFont:AUTO(10) andWeight:1 andTextColor:COLOR_2F2F2F andBackgroundColor:JCClearColor andTextAlignment:NSTextAlignmentCenter];
...
- (UILabel *)xdLab {
    if (!_xdLab) {
        _xdLab = [UILabel initWithTitle:@"" andFont:AUTO(10) andWeight:1 andTextColor:COLOR_2F2F2F andBackgroundColor:JCClearColor andTextAlignment:NSTextAlignmentCenter];
